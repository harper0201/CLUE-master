...
def _renew_offset(all_doc_tokens, offset):
    traverse_from_end = offset
    while traverse_from_end > 0:
        if all_doc_tokens[traverse_from_end] == '。' or all_doc_tokens[traverse_from_end] == '!' or all_doc_tokens[traverse_from_end] == '?' or all_doc_tokens[traverse_from_end] == ';' or all_doc_tokens[traverse_from_end] == '，':
            offset = traverse_from_end
            break
        elif all_doc_tokens[traverse_from_end] == ')' and all_doc_tokens[traverse_from_end - 1] == '。':
            offset = traverse_from_end
            break
        elif all_doc_tokens[traverse_from_end] == '。' and all_doc_tokens[traverse_from_end - 1] == '”':
            offset = traverse_from_end
...
def json2features(input_file, output_files, tokenizer, is_training=False, repeat_limit=3, max_query_length=64,
    ...
    # the fuhao marks the end of sentence
    def _end_of_sentence(c):
        if c == '。' or c == '！' or c == '？' or c == '；':
            return True
        return False
    ...
    for (example_index, example) in enumerate(tqdm(examples)):
        ...
        while start_offset < len(all_doc_tokens):
            ...
            if start_offset != 0 and start_offset != len(all_doc_tokens)-1:
                start_offset = _renew_offset(all_doc_tokens, start_offset) + 1
            orig_end_offset = min(start_offset + length - 1, len(all_doc_tokens)-1)
            end_offset = _renew_offset(all_doc_tokens, min(start_offset + length - 1, len(all_doc_tokens)-1))
            if start_offset > end_offset:
                end_offset = orig_end_offset
            if end_offset - start_offset < length:
                length = end_offset - start_offset
            doc_spans.append(_DocSpan(start=start_offset, length=length))
            if start_offset + length == len(all_doc_tokens)-1:
                break
